...
//FLASHÆðÊ¼µØÖ·
#define STM32_FLASH_BASE 	   0x08000000 			/*STM32 FLASH Begain Address*/
#define FLASH_STATE_ADDR  	   ADDR_FLASH_PAGE_15   /* ADDRESS to write IAP status */
#define FLASH_USER_START_ADDR  ADDR_FLASH_PAGE_16   /* Start @ of user Flash area */
#define FLASH_DL_START_ADDR    ADDR_FLASH_PAGE_130  /* Copy @ of user Flash area */


//#define FLASH_USER_END_ADDR    ADDR_FLASH_PAGE_255 + FLASH_PAGE_SIZE - 1   /* End @ of user Flash area */

//#define FLASH_USER_END_ADDR    ADDR_FLASH_PAGE_150 + FLASH_PAGE_SIZE - 1   /* End @ of user Flash area */
#define FLASH_USER_END_ADDR    ADDR_FLASH_PAGE_246 + FLASH_PAGE_SIZE - 1   /* End @ of user Flash area   F12050D*/

uint8_t ulCopy_Code_Flash(uint32_t ulLength);
uint8_t ucFLASH_USER_PAGE_PROGRAM(uint32_t ulStartAddress,uint8_t* pbuff,uint32_t ulLength);/*FLASH page erase and program*/
uint8_t ucFLASH_ERASE(uint32_t ulStartAddr,uint32_t ulLength);/*FLASH ERASE*/
uint8_t ucFLASH_PROGRAM(uint32_t ulStartAddr,uint8_t* pbuff,uint32_t ulLength);/*FLASH WRITE*/
uint64_t ulByteToDoubleword(uint8_t* data,uint8_t ucLen);/*bytes to doubleword writed to flash*/
